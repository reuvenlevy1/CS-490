...
//Input for Creating Questions to be inputted into DB page--------------------
  if($_POST['type']=='create_questions'){                                     //1 question passed from Frontend $data = array ('problem'=>$problem, 'difficulty'=>$difficulty, 'points'=>$points, 'topic'=>$topic, 'test_case_1'=>$test_case_1,..., 'test_case_5=>'...');
    ...
    //echo "database executed";
  }
//==============================================================================

//$delete_question_id;
//$data = array("type"=>"exam_questions","delete_question_id"=>$delete_question_id);
  ...
  //if($data['type']=='exam_questions'){
    ...
    if(isset($_POST['delete_question_id'])){
        ...
        if(!empty($delete_question_id)){
          ...
          // $query = "DELETE FROM questions WHERE id=$delete_question_id";      //Deletes single question by id
...
//Input for Created Exam from Professor's page--------------------------------//Receiving $data = array("type"=>"exam_created", "id"=>$id_string, "question_points"=>$points_string);
  if($_POST['type']=='exam_created'){                                         //Chosen questions for exam
    ...
    //echo $id_string;
    // $tmp_id_array = explode(",",$id_string);                                  //Takes values separated by commas in string and puts into an array
    // //take out values that are equal to null from id_array
    // $j = 0;                                                                   //index of id_array
    // $id_array = [];                                                           //declare id_array
    // //id_array will have only id values, no null values.
    // for($i=0; $i<sizeof($tmp_id_array); $i++){
    //   if($tmp_id_array[$i] != "null"){
    //     $id_array[$j] = $tmp_id_array[$i];
    //     $j++;
    //   }
    // }
    ...
    for($i=0; $i<$id_string_size; $i++){
      //$query_insert = "INSERT INTO exam (id) VALUES (".$id_array[$i].")";
      //echo " |id_array_size loop: ".$i."| ";
      $query = "UPDATE questions SET approved='y' WHERE id IN (".$id_string[$i].")";
      //$query_run = mysqli_query($con, $query) or die(mysqli_error($con));   //running this makes it die and doesn't move on even though the query technically works.
      $query_run = mysqli_query($con, $query);
    ...
    //   //$query_insert = "INSERT INTO exam (id) VALUES (".$id_array[$i].")";
    //   //echo " |id_array_size loop: ".$i."| ";
    //   $query = "UPDATE questions SET approved='y' WHERE questions.id IN (".$id_array[$i].")";
    //   //$query_run = mysqli_query($con, $query) or die(mysqli_error($con));   //running this makes it die and doesn't move on even though the query technically works.
    //   $query_run = mysqli_query($con, $query);
    // }

    //==========================FORGET EXAM ID FOR NOW==========================
    ...
    //switch "approved" in "question" table based on id to "yes" for all ids that match those in exam table

    //$query = "UPDATE questions SET approved='y' WHERE questions.id IN (SELECT id FROM exam)";   //Changes approved value in "questions" table to all the ids that match with those in "exam" table
    //$query_run = mysqli_query($con, $query);
  }
...
//Input for Student's Exam from student's page--------------------------------//Receiving $data = array("type"=>"student_exam");
  if($_POST['type']=='student_exam'){                                         //Creates exam questions and related info to Student's page
    //Make arrays based on those in "question" table that are marked "yes" for "approved" column
...
//$username is the name of the student's username; This should match the id's in the "exam" table. $answers is the student's answers to those questions. Indexes should match up.
  if($_POST['type']=='student_answers'){                                      //Creates a new table named after username storing answers to each question
    ...
    //username,question_id,student's_code
    ...
    //print_r($points);
    //echo " test case 1 answer: ";
    //print_r($test_case_1_answer);
...
//Add the commented values to the for loop below, all incrementing by $i
...
//array('type'=>'points_update','username'=>$username,'exam_id'=>$exam_id,'question_id'=>$question_id,'points'=>$points);
    ...
    if(isset($_POST['question_grade'])){
      ...
      for($i=0; $i<sizeof($question_grade); $i++){
        if(preg_match('/^[0-9]/', $question_grade[$i])){
          ...
          //query: UPDATE `questions` SET `test_case_".$i."`='".$passed_array["test_case_$i"]."' WHERE `questions`.`id`=".$result_id[0];
    ...
    // for($i=0; $i<sizeof($question_id); $i++){                                 //Comments per question
    //   $query = "UPDATE answers SET comments='$comments[$i]' WHERE username='$username' AND exam_id='$exam_id' AND question_id='$question_id_array[$i]'";
    //   $query_run = mysqli_query($con, $query);
    // }
    ...
    $query_run = mysqli_query($con, $query) or die(mysqli_error($con));
    mysqli_close($con);                                                       //Closes database connection
    //INSERT INTO `answers` (`username`, `exam_id`, `question_id`, `student_code`, `test_case_1_answer`, `test_case_2_answer`, `test_case_3_answer`, `test_case_4_answer`, `test_case_5_answer`, `points`) VALUES ('Reuven', '1', '2', 'Hello over there', 'divTwo(10,5)   2', '', '', '', '', '90');
  }
//==============================================================================
...
//truncate tables from DB --> just comment out if statement and run in AFS to executed
  if($_POST['type']=='reset'){
    $query = "TRUNCATE TABLE answers";
    $query_run = mysqli_query($con, $query);

    $query = "TRUNCATE TABLE exams";
    $query_run = mysqli_query($con, $query);

    $query = "TRUNCATE TABLE exam_grades";
    $query_run = mysqli_query($con, $query);

    $query = "TRUNCATE TABLE exam_grades";
    $query_run = mysqli_query($con, $query);

    $query = "TRUNCATE TABLE view_results";
    $query_run = mysqli_query($con, $query);

    $query = "UPDATE questions SET approved='n'";
    $query_run = mysqli_query($con, $query);

    $query = "UPDATE questions SET points='0'";
    $query_run = mysqli_query($con, $query);
